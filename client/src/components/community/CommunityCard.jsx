...
const CommunityCard = ({ community }) => {
  ...
  return (
    <div className="px-3 py-3 rounded-md border bg-white shadow-2xl shadow-[#f2f5fc] flex justify-between">
      ...
      <div className="">
        <button
          onClick={() => toggleJoinModal(community.id, true)}
          className="px-2.5 py-2.5 bg-primary bg-blue-800 shadow-2xl shadow-[#F3F8FF] hover:bg-transparent group hover:border rounded-xl hover:border-primary transition duration-300"
        >
          <MdOutlineGroupAdd className="text-lg text-white group-hover:text-primary" />
        </button>
        <JoinModal
          show={joinModalVisibility[community.id] || false}
          onClose={() => toggleJoinModal(community.id, false)}
          community={community}
        />
      </div>
    </div>

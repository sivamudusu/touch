...
import { getPostAction, clearPostAction } from "../redux/actions/postAction";
...
const Post = () => {
  ...
  const joinedCommunities = useSelector((state) =>
    state.community?.joinedCommunities?.map(({ id }) => id)
  );
  ...
  const isAuthorized = useMemo(() => {
    return post && joinedCommunities?.includes(post.communityId);
